...
std::optional<SignParams> ParseEthSignParams(const base::Value::List& params) {
  if (params.size() != 2) {
    return std::nullopt;
  }
...
std::optional<SignParams> ParsePersonalSignParams(
    const base::Value::List& params) {
...
std::optional<EcRecoverParams> ParsePersonalEcRecoverParams(
    const base::Value::List& params) {
